...
func newTestScheduler(conf *testSchedulerConf) *testScheduler {
	return &testScheduler{
		conf: conf,
	}
}

func (ts *testScheduler) Start(job scheduledJob) {
	jobrunner.Start()

	jobrunner.Now(job)
	jobrunner.Schedule(ts.conf.Schedule, job)
	http.HandleFunc("/status", ts.statusHandler)
	go http.ListenAndServe(ts.conf.WebConsoleEndPoint, nil)
}

func (ts *testScheduler) statusHandler(w http.ResponseWriter, r *http.Request) {
